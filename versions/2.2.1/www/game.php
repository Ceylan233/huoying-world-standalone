...
if (!isset($_SESSION['userId']) || !isset($_SESSION['username'])) {
    echo "กรุณาล็อกอินก่อนเข้าเกม";  // ถ้าไม่ได้เข้าสู่ระบบให้แสดงข้อความ
    exit();
}

$userId = $_SESSION['userId'];
$username = $_SESSION['username'];
$password = $_SESSION['password'];  // เก็บข้อมูลที่ต้องการส่งไปยัง Flash
?>
...
//	console.log(fls)
  ...
  // 处理从Flash客户端发来的GameServer连接请求
  function connectToGameServer(ip, port, clientId, channelId, timeStamp) {
    console.log("========== 收到GameServer连接请求 ==========");
    console.log("  IP: " + ip);
    ...
    // TODO: 这里需要加载Main.swf并连接到GameServer
    // 可能的实现方式：
    // 1. 刷新页面并传递GameServer连接参数
    // 2. 动态加载Main.swf并传递连接参数
    // 3. 使用LocalConnection在两个SWF之间通信
  }
  ...
  var params = { 
	// 核心连接参数（必需）
	ip:'127.0.0.1',      // TCP服务器IP
	port:'18684',         // TCP服务器端口
	baseUrl:'127.0.0.1:18680/act_web_tiyan', // 资源文件基础URL，客户端会自动加上"http://"前缀变成"http://127.0.0.1:18680/act_web_tiyan/"
	...
	time:'',             // 时间戳
	sign:'',            // 签名
	password:'<?php echo $password; ?>',         // 密码（可选）
	...
	gameSwitch:'lunhuiyan:0_kunchong:0_jieyin:0_zhongrenkaoshi:0_huoyuedu:1_chongwuroughun:0_famillyshop:0_juedou:0', // 游戏开关
